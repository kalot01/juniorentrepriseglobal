...
function getFields() {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      html +=
        '<option value="">Others (Fields currently Undefined on profile)</option>';
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (var i = 0; i < mapdb.length; i++) {
        // Process country names
        mapdb[i].country = mapdb[i].country
          .replace("danemark", "denmark")
          .replace("-union", "")
          .replace("usa", "united-states")
          .replace("_", " ")
        ...
        if (
          mapdb[i].fields_id !== null &&
          typeof mapdb[i].fields_id === "object"
        ) {
...
function runSearch() {
  ...
  for (var i = 0; i < mapdb.length; i++) {
    if (
      (mapdb[i].confederation == 0 && listJEs) ||
      (mapdb[i].confederation == 1 && listConfs)
    ) {
  ...
  var resultsHTML = result
    .map(function (je) {
      // Name
      var h =
        '<button class="result-container" onclick="openInfo(&quot;' +
        je.id +
        '&quot;)"><div class="result">' +
        '<div class="je-name">' +
        je.name +
...
function openInfo(je) {
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      document.getElementById("je-logo").style.backgroundImage =
        "url(https://junior-connect.com/storage/" + jeinfo.logo + ")";
      // Name
      document.getElementById("je-name").innerHTML = jeinfo.name;
      // Location
      document.getElementById("je-location").innerHTML =
        jeinfo.city + " – " + jeinfo.country;
      // Universities
      if (jeinfo.universities.length != 0) {
        if (jeinfo.universities.length == 1) {
          document.getElementById("je-uni").innerHTML =
            '<h3 class="je-subtitle">UNIVERSITY</h3>' + jeinfo.universities[0];
        } else {
          document.getElementById("je-uni").innerHTML =
            '<h3 class="je-subtitle">UNIVERSITIES</h3>' +
            jeinfo.universities[0];
          for (var i = 1; i < jeinfo.universities.length; i++) {
            document.getElementById("je-uni").innerHTML +=
              "<br>" + jeinfo.universities[i];
          }
        }
      } else {
        document.getElementById("je-uni").innerHTML = "";
        ...
        if (jeinfo.domains.length == 1) {
          if (jeinfo.domains[0].field != null) {
            document.getElementById("je-field").innerHTML =
              '<h3 class="je-subtitle">FIELD</h3>' + jeinfo.domains[0].field;
          } else {
            document.getElementById("je-field").innerHTML = "";
          }
        } else {
      ...
      if (!(jeinfo.confederation == null || jeinfo.confederation == "")) {
        document.getElementById("je-parent").innerHTML =
          '<h3 class="je-subtitle">CONFEDERATION</h3>' + jeinfo.confederation;
      } else {
        document.getElementById("je-parent").innerHTML = "";
      }
      // Stats
      if (
        jeinfo.nbr_members != 0 ||
        jeinfo.nbr_projects != 0 ||
        jeinfo.nbr_ca != 0
      ) {
        var part1 =
          '<h3 class="je-subtitle">STATS</h3><div class="stats-container">';
        var part2 = "";
        if (jeinfo.nbr_members != 0) {
          part2 +=
            '<div class="child"><div class="stat">' +
            jeinfo.nbr_members +
            '</div><div class="description">Team Members</div></div>';
        }
        if (jeinfo.nbr_projects != 0) {
          part2 +=
            '<div class="child"><div class="stat">' +
            jeinfo.nbr_projects +
            '</div><div class="description">Projects in 2018</div></div>';
        }
        if (jeinfo.nbr_ca != 0 && Math.round(jeinfo.nbr_ca / 1000) != 0) {
          part2 +=
            '<div class="child"><div class="stat">' +
            Math.round(jeinfo.nbr_ca / 1000) +
            'k</div><div class="description">Revenue in ' +
            jeinfo.devise +
            " in 2018</div></div>";
        }
      ...
      var contact =
        '<h3 class="je-subtitle">CONTACT</h3><div class="card contact-card">';
      if (jeinfo.website != null) {
        contact +=
          '<a href="' + jeinfo.website + '"><i class="fa fa-globe"></i></a>';
        hascontact = true;
      }
      if (jeinfo.phone != null) {
        contact +=
          '<a href=tel:"' +
          jeinfo.phone.replace(/\s+/g, "-") +
          '" class="ttipx"><i class="fa fa-phone"></i><div class="tooltiptext">' +
          jeinfo.phone +
          "</div></a>";
        hascontact = true;
      }
      if (jeinfo.email != null) {
        contact +=
          '<a href=mailto:"' +
          jeinfo.email +
          '" class="ttipx"><i class="fa fa-envelope"></i><div class="tooltiptext">' +
          jeinfo.email +
          "</div></a>";
        hascontact = true;
        document.getElementById("jc-access").innerHTML =
          "<b>Do you have access to " +
          jeinfo.email +
          '?</b><br><a href="https://junior-connect.com/globalcouncil/organization/' +
          jeinfo.id +
          '/access" target="_blank">Click here to create a JuniorConnect account</a></p>';
      }
      contact += "</div>";
      if (hascontact) {
      ...
      var hassocial = false;
      var social =
        '<h3 class="je-subtitle">SOCIAL</h3><div class="card contact-card">';
      ...
      var hasbench = false;
      var bench =
        '<h3 class="je-subtitle ttip">BENCHMARKING CONTACT<div class="tooltiptext">Use this contact if you&apos;re a Junior Entrepreneur that wants to contact this JE to exchange ideas, practices and knowledge!</div></h3><div class="bench-contact">';
      // Benchmarking Contact
      if (jeinfo.contact != null) {
        if (jeinfo.contact.name != null) {
          hasbench = true;
          bench +=
            '<div class="child-60"><div class="contact-name">' +
            jeinfo.contact.name +
            "</div>";
        }
...
function closeInfo() {
  ...
  document.getElementById("search").className = document
    ...
    .className.replace(/compressed/g, "");
  document.getElementById("map").className = document
    .getElementById("map")
    .className.replace(/compressed/g, "");
  google.maps.event.trigger(map, "resize");
  map.fitBounds(bounds);
  ...
  document.getElementById("search").className = document
    .getElementById("search")
    .className.replace(/mobile-hidden/g, "");
}
...
function colorNav() {
  ...
  if (color != "rgb(36, 161, 200)") {
    if (scroll > 136) {
      ...
      document.getElementById("je-info-nav").style.borderBottom =
        "6px solid #1f8cad";
    }
  } else {
    if (scroll <= 136) {
      document.getElementById("je-info-nav").style.background =
